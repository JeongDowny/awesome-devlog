...
> Claude Code 세션 로그를 자동으로 수집·구조화하여 devLog를 생성하는 CLI 도구

Claude Code로 작업하면서 쌓인 대화 로그와 git 이력을 자동으로 분석하여, **문제 → 원인 → 해결 → 결과** 구조의 devLog 초안을 만들어줍니다. 사용자는 핵심 맥락만 보완하면 됩니다.

## 만들게 된 이유

하루 종일 Claude Code와 대화하면서 일하고 나면, 퇴근할 때 "오늘 뭐 했지?" 싶을 때가 있습니다.
문제 해결 과정을 기록하고 싶은데, 일하면서 동시에 정리하긴 현실적으로 어렵고요.

그런데 생각해보면 Claude Code 대화 로그에 이미 모든 맥락이 다 담겨 있습니다.
그걸 자동으로 꺼내서 구조화해주면, 하루 5분이면 devLog를 완성할 수 있지 않을까 — 그래서 만들었습니다.
...
devLog 저장 경로를 설정합니다. 설정은 `~/.devlog/config.json`에 저장됩니다.
