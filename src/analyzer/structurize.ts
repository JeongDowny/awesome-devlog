...
주어진 데이터:
- Claude Code 또는 Codex CLI 대화 로그 (사용자 질문 + AI 응답) — 두 출처가 섞여 있을 수 있음
- git commit 이력
...
- 같은 작업이 여러 세션(Claude/Codex)에 걸쳐 있으면 하나의 태스크로 병합하세요. 사용된 도구(Claude/Codex)는 최종 출력에 표기하지 않습니다 — 작업 내용 중심으로 정리하세요.
...
export type StructurizeProgress = {
  taskCount: number;
  chars: number;
};
 ...
 * onProgress가 주어지면 스트리밍 응답을 바탕으로 누적 태스크 수와 문자 수를 보고한다.
 */
export async function structurizeTasks(
  data: CollectedData,
  onProgress?: (state: StructurizeProgress) => void,
): Promise<Task[]> {
  ...
  const response = await askClaude(prompt, {
    onDelta: onProgress
      ? (_delta, accumulated) => {
          // "title" 키 등장 횟수로 생성 중인 태스크 수를 근사치로 추정
          const taskCount = (accumulated.match(/"title"\s*:/g) ?? []).length;
          onProgress({ taskCount, chars: accumulated.length });
        }
      : undefined,
  });
 ...
 * onProgress는 지금까지 수집된 제안 문자열 개수를 보고한다.
 */
export async function suggestLessons(
  tasks: Task[],
  onProgress?: (state: { lessonCount: number; chars: number }) => void,
): Promise<string[]> {
...
순수 JSON 배열만 응답해줘 (마크다운 코드블록 없이): ["배운 점 1", "배운 점 2", ...]`;
  ...
  const response = await askClaude(prompt, {
    onDelta: onProgress
      ? (_delta, accumulated) => {
          // 배열 안의 문자열 항목 개수를 콤마+따옴표 패턴으로 근사
          const lessonCount = (accumulated.match(/",\s*"/g) ?? []).length + (accumulated.includes('["') ? 1 : 0);
          onProgress({ lessonCount, chars: accumulated.length });
        }
      : undefined,
  });
 ...
 * onProgress가 주어지면 수신한 문자 수를 보고한다.
 */
export async function reviseMarkdown(
  currentMarkdown: string,
  userPrompt: string,
  onProgress?: (state: { chars: number }) => void,
): Promise<string> {
...
- 한국어로 작성하세요.`;
  ...
  return askClaude(prompt, {
    onDelta: onProgress
      ? (_delta, accumulated) => {
          onProgress({ chars: accumulated.length });
        }
      : undefined,
  });
}
  ...
  for (const project of data.projects) {
    ...
    for (const session of project.sessions) {
      ...
      if (content) {
        ...
        parts.push(`### 세션 [${sourceLabel}] (${startTime} ~ ${endTime})\n${content}`);
      }
    }
